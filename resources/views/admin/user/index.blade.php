		        ...
		        <tbody role="alert" aria-live="polite" aria-relevant="all">
		        	@foreach($users as $k=>$v)
			          <tr class="@if($k%2==1) odd @else even @endif">
			            ...
			            <td class=" ">
			            	<a class="btn btn-info icon-pencil" href="/user/edit?id={{$v->id}}"></a>
	            			<a class="btn btn-danger icon-trash" href="/user/delete?id={{$v->id}}"></a>
			            </td>
			          </tr>
			        @endforeach
		        </tbody>
	...
	<script type="text/javascript">
		$(function(){
			...
			$('.ibutton-container').click(function(){
				...
				//发送ajax请求
				$.get('/user/aupdate',{'status':status,'id':id},function(data){
					if(data == 1){
						success('更新成功');
					}else{
						error('更新失败');
